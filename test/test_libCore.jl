...
#TODO: Add tests that core functions (used in hot loops etc.) are inferred correctly
# using @inferred
@testset "Core" begin
    ...
    @testset "findBoundsUnivX" begin
        @test_broken isapprox(findBoundsUnivX(selectmap("Log")[1], selectmap("Log")[3], 0.80901699437494739575171820433752,1)[1],-0.3088088088088088)
        @test_broken isapprox(findBoundsUnivX(selectmap("Cub")[1], selectmap("Cub")[3], 0.81649658092772603273242802490196,1)[1], -0.316089007928365)
        @test_broken isapprox(findBoundsUnivX(selectmap("Sin")[1], selectmap("Sin")[3], 0.7777337661716061112393560961209,1)[1], -0.277777777777778)
        @test_broken isapprox(findBoundsUnivX(selectmap("Qua")[1], selectmap("Qua")[3], 0.90958625669808221984978559417121,1)[1], -0.409909909909910)
    end
