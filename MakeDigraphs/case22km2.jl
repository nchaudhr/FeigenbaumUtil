...
\\end{tikzpicture}%}
\\caption{Digraph of cyclic permutation~\\cref{eq:valid322km2-}, in setting \$(2,k-1,2k-2)\$.}
\\label{fig:valid322km2-}
\\end{figure}"

permstr = "\\small{
\\begin{equation}
\\left(\\begin{array}{cccccccccccccccc}
1 & 2 & 3 & \\cdots & k-1 & k & k+1 & k+2 & \\cdots & 2k-1 & 2k & 2k+1 \\\\
<INSERT PERM HERE>
\\end{array} \\right)
\\label{eq:valid322km2-}
\\end{equation}}"
...
ctr = 1
for perm in perms
	savFile = joinpath(dirname(@__FILE__),"digraph-valid322km2-$(ctr).tex")
	pth = "\n"
	for idx in spots
	...
	digtex = replace(replace(digstr,"<INSERT PATH HERE>",pth),":valid322km2-",":valid322km2-$(ctr)")
	...
	permtex = replace(replace(permstr,"<INSERT PERM HERE>",pth),":valid322km2-",":valid322km2-$(ctr)")
	println(permtex)
	println("\n")
