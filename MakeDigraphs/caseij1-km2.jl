...
\\end{tikzpicture}%}
\\caption{Digraph of cyclic permutation~\\cref{eq:valid3ij1km2-}, in setting \$(k-2,k-1,k+1)\$.}
\\label{fig:valid3ij1km2-}
\\end{figure}"

permstr = "\\small{
\\begin{equation}
\\left(\\begin{array}{cccccccccccccccc}
1 & \\cdots & k-3 & k-2 & k-1 & k & k+1 & k+2 & k+3 & k+4 & k+5 & \\cdots \\\\
<INSERT PERM HERE>
\\end{array} \\right)
\\label{eq:valid3ij1km2-}
\\end{equation}}"
...
ctr = 1
for perm in perms
	savFile = joinpath(dirname(@__FILE__),"digraph-3ij1km2-$(ctr).tex")
	pth = "\n"
	for idx in spots
	...
	digtex = replace(replace(digstr,"<INSERT PATH HERE>",pth),":valid3ij1km2-",":valid3ij1km2-$(ctr)")
	...
	permtex = replace(replace(permstr,"<INSERT PERM HERE>",pth),":valid3ij1km2-",":valid3ij1km2-$(ctr)")
	println(permtex)
	println("\n")
