...
\\end{tikzpicture}}
\\caption{Digraph of cyclic permutation~\\cref{eq:valid3ij1}, in setting \$(i,k-1,j), 2 < i < k-2\$.}
\\label{fig:valid3ij1}
\\end{figure}"
...
1 & \\cdots & i-1 & i & i+1 & i+2 & \\cdots & k-1 & k & k+1 & k+2 & \\cdots & j+1 & j+2 & j+3 & \\cdots \\\\
<INSERT PERM HERE>
\\end{array} \\right)
\\label{eq:valid3ij1}
\\end{equation}}"
...
ctr = 1
for perm in perms
	savFile = joinpath(dirname(@__FILE__),"digraph-3ij1-$(ctr).tex")
	pth = "\n"
	for idx in spots
	...
	digtex = replace(replace(digstr,"<INSERT PATH HERE>",pth),":valid3ij1",":valid3ij1$(ctr)")
	...
	permtex = replace(replace(permstr,"<INSERT PERM HERE>",pth),":valid3ij1",":valid3ij1$(ctr)")
	println(permtex)
	println("\n")
